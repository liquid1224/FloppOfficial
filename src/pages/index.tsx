    ...
    return (
      <div className={Vanilla.HeroInfo}>
        <div className={Vanilla.HeroWorks}>
          {data.works.edges.map((node, index) => {
        ...
        <div className={Vanilla.BlogPosts}>
          ...
          <ul className={Vanilla.BlogPostsUl}>
            {data.articles.edges.map((node, index) => {
              ...
              const title = node.node.frontmatter?.title as string;
              return (
                <li className={Vanilla.BlogPostLi}>
                  <Link to={`/blog/${slug}`} key={index} className={`${Vanilla.BlogPostLink} ${isDarkMode ? Vanilla.BlogPostLinkDark : ""}`}>
                    {title}
                  </Link>
                </li>
              );
      ...
      <>
        ...
        <div className={Vanilla.IndexContentsWrapper}>
          ...
          <div className={Vanilla.Section}>
            <div className={Vanilla.MusicTextWrapper}>
              <h2 className={Vanilla.Header}>Hardware & Software</h2>
...
export const query = graphql`
  query HeroInfo {
    works: allMarkdownRemark(sort: { frontmatter: { date: DESC } }, filter: { fileAbsolutePath: { regex: "/works/" }, frontmatter: { index: { eq: "1" } } }, limit: 4) {
      edges {
        node {
          frontmatter {
            title
    ...
    articles: allMarkdownRemark(filter: { fileAbsolutePath: { regex: "/articles/" } }, sort: { frontmatter: { date: DESC } }, limit: 3) {
      edges {
        node {
          frontmatter {
            slug
